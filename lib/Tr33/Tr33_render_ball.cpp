...
void Tr33::render_ball(uint8_t strip_index, int ball_type, float center, float width, CRGB color, float ball_brightness, bool bounce_top, bool bounce_bottom)
{
  ...
  case BALL_TYPE_COMET:
    ...
    break;
  case BALL_TYPE_NYAN:
    render_nyan(strip_index, center, width * 10.0, color, ball_brightness, bounce_top, bounce_bottom);
    break;
...
void Tr33::render_sine_ball(uint8_t strip_index, float center, float width, CRGB color, float ball_brightness)
{
  int strip_length = strip_index_length(strip_index);
  int start_led = max(0, int(ceilf(center - width / 2.0)));
  int end_led = min(strip_length, int(floorf(center + width / 2.0)));
...
void Tr33::render_comet(uint8_t strip_index, float center, float length, CRGB color, float ball_brightness, bool bounce_top, bool bounce_bottom)
{
...
void Tr33::render_nyan(uint8_t strip_index, float center, float length, CRGB color, float ball_brightness, bool bounce_top, bool bounce_bottom)
{

  render_sine_ball(strip_index, center, 3, ColorFromPalette(currentPalette, 0), ball_brightness);
