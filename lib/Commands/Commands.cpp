...
void Commands::run()
{
  ...
  if (run_commands)
  {
    for (int i = 0; i < COMMAND_BUFFER_SIZE; i++)
    {
      ...
      {
      case SINGLE_COLOR:
        single_color(command_buffer[i].data);
        break;
      case WHITE:
        all_white();
        break;
      case RAINBOW_SINE:
        rainbow_sine(command_buffer[i].data);
        break;
      case PING_PONG:
        ping_pong(command_buffer[i].data);
        break;
      case GRAVITY:
        gravity(command_buffer[i].data);
        break;
      case SPARKLE:
        sparkle(command_buffer[i].data);
        break;
      case SHOW_NUMBER:
        show_number(command_buffer[i].data);
        break;
      case RAIN:
        rain(command_buffer[i].data);
        break;
      case BEATS:
        beats(command_buffer[i].data);
        break;
      case MAPPED_SWIPE:
        mapped_swipe(command_buffer[i].data);
        break;
      case MAPPED_SHAPE:
        mapped_shape(command_buffer[i].data);
        break;
      }
    }
  }
